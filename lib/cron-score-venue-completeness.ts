...
const ROUTE = "/api/cron/venues/score-completeness";
const CRON_NAME = "score_venue_completeness";
const BATCH_SIZE = 50;
...
export async function runCronScoreVenueCompleteness(
  ...
  const lock = await tryAcquireCronLock(db, "cron:venue:score-completeness");
  if (!lock.acquired) {
    const summary = {
      ok: false,
    ...
    for (let i = 0; i < venues.length; i += BATCH_SIZE) {
      ...
      const results = await Promise.allSettled(
        batch.map(async (venue) => {
          const result = computeVenueCompleteness({
